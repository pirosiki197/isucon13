...
func getReactionsHandler(c echo.Context) error {
	...
	for i := range reactionModels {
		reaction, err := fillReactionResponseDB(ctx, dbConn, reactionModels[i])
		if err != nil {
...
func fillReactionResponseDB(ctx context.Context, db *sqlx.DB, reactionModel ReactionModel) (Reaction, error) {
	userModel := UserModel{}
	if err := db.GetContext(ctx, &userModel, "SELECT * FROM users WHERE id = ?", reactionModel.UserID); err != nil {
		return Reaction{}, err
	}
	user, err := fillUserResponseDB(ctx, db, userModel)
	if err != nil {
		return Reaction{}, err
	}
